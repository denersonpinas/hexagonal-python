...
class FindCounterpart(FindCounterpartInterface):
    ...
    def by_id(self, counterpart_id: int) -> Dict[bool, List[Counterpart]]:
        ...
        if validate_entry:
            response = self.counterpart_repository.select_counterpart(
    ...
    def by_id_and_required_and_default(
        ...
        if validate_entry:
            response = self.counterpart_repository.select_counterpart(
                counterpart_id=counterpart_id, required=required, default=default
            )

        return {"Success": validate_entry, "Data": response}

    def all(self) -> Dict[bool, List[Counterpart]]:
        """Select all Counterpart
        :param  -   is None
        :return -   Dictionary with informations od the process
        """

        response = self.counterpart_repository.select_all_counterpart()
