...
faker = Faker("pt_BR")
supply_history = SupplyHistoryRepository()
db_connection_handler = DBConnectionHandler()
engine = db_connection_handler.get_engine()


def test_insert_supply_history():
    """Should insert Supply History"""
    ...
    with DBConnectionHandler() as db_connection:
        try:
            ...
            new_history = supply_history.insert_supply_history(
                service_provided=service_provided,
                hiring_manager=hiring_manager,
                proposal_id=proponent.proposta_id,
            )

            # Select Supply History
            query = select(HistoricoDeFornecimento).where(
                HistoricoDeFornecimento.id == new_history.id
            )

            for query_history in db_connection.session.execute(query):
                assert new_history.id == query_history[0].id
                assert new_history.servico_prestado == query_history[0].servico_prestado
                assert (
                    new_history.responsavel_contratacao
                    == query_history[0].responsavel_contratacao
                )
                assert new_history.proposta_id == query_history[0].proposta_id

            # Deleting Supply History Inserted
            history_inserted = db_connection.session.get(
                HistoricoDeFornecimento, new_history.id
            )
